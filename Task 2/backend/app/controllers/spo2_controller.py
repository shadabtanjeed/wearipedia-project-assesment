...
def get_all_spo2_data(
    ...
    try:
        ...
        if not results:
            logger.info(
                f"No SpO2 data for user {user_id} between {start_date} and {end_date}. Trying fallback."
            )
            fallback_query = """
            SELECT 
                timestamp,
            ...
            if not results:
                ...
                if not user_exists:
                    ...
                    logger.warning(f"User {user_id} exists but has no SpO2 data")
                    return []

        return results

    except Exception as e:
        logger.error(f"Database error in get_all_spo2_data: {str(e)}")
        return []
    finally:
        if cursor:
            cursor.close()
...
def get_daily_avg_spo2_data(
    ...
    try:
        ...
        if not results:
            logger.info(
                f"No daily avg SpO2 data for user {user_id} between {start_date} and {end_date}. Trying wider range fallback."
            )
            ...
            if not results:
                ...
                if not user_exists:
                    ...
                    logger.warning(
                        f"User {user_id} exists but has no daily average SpO2 data"
                    )
                    return []
        ...
        logger.error(f"Database error in get_daily_avg_spo2_data: {str(e)}")
        return []
    finally:
        if cursor:
            cursor.close()
